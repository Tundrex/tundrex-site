...
(function () {
  ...
  var CartManager = {
    ...
    /* ── Checkout via Shopify Cart API + shop.app ── */
    /* tundrex.co DNS points to Netlify, not Shopify, so the Storefront API
       checkoutUrl (tundrex.co/cart/c/...) hits a Netlify 404.
       Fix: create cart via Storefront API, extract the cart token,
       then redirect to shop.app/checkout/SHOP_ID/cn/TOKEN/LOCALE
       which serves the Shopify checkout directly — no domain conflict. */
    checkout: function () {
      var self = this;
      ...
      gql(mutation, { input: { lines: lines } })
        .then(function (data) {
          ...
          if (result && result.cart && result.cart.id) {
            self.cartId = result.cart.id;
            self._save();
            /* Extract cart token from GID:
